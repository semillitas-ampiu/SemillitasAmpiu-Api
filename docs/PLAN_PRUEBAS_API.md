...
| Base de Datos     | PostgreSQL con al menos un `Nivel` creado (ID=1) |
| Herramientas      | pytest, requests, pytest-order                   |
...
> Como administrador, quiero gestionar el vocabulario del idioma Ampiu asociando palabras en español con su traducción y asignándolas a niveles de dificultad.
...
| P02 | Crear palabra completa          | test_P02_crear_palabra_completa          | Crea una palabra con español, ampiu y nivel   | \* Nivel existente (ID=1)              | CRUD Palabra   | 1. POST a `/api/palabra/` con datos completos        | `{"pal_español": "hola", "pal_ampiu": "kua", "nivel": 1}`      | Status 201, mensaje="Palabra creada correctamente." | Nivel ID=1   | QA Team     |
| P03 | Crear palabra sin nivel         | test_P03_crear_palabra_sin_nivel         | Rechaza creación cuando falta el nivel        | \* Ninguna                             | CRUD Palabra   | 1. POST sin campo nivel                              | `{"pal_español": "test", "pal_ampiu": "test"}`                 | Status 400 Bad Request                              | Ninguna      | QA Team     |
| P04 | Crear palabra nivel inexistente | test_P04_crear_palabra_nivel_inexistente | Rechaza creación con nivel que no existe      | \* Ninguna                             | CRUD Palabra   | 1. POST con nivel=99999                              | `{"pal_español": "test", "pal_ampiu": "test", "nivel": 99999}` | Status 400 Bad Request                              | Ninguna      | QA Team     |
| P05 | Crear palabra español duplicado | test_P05_crear_palabra_español_duplicado | Rechaza creación cuando pal_español ya existe | \* Palabra con mismo español existente | CRUD Palabra   | 1. Crear palabra 2. Crear otra con mismo pal_español | pal_español duplicado                                          | Status 400 Bad Request                              | P02          | QA Team     |
| P06 | Crear palabra ampiu duplicado   | test_P06_crear_palabra_ampiu_duplicado   | Rechaza creación cuando pal_ampiu ya existe   | \* Palabra con mismo ampiu existente   | CRUD Palabra   | 1. Crear palabra 2. Crear otra con mismo pal_ampiu   | pal_ampiu duplicado                                            | Status 400 Bad Request                              | P02          | QA Team     |
...
> Como sistema, necesito registrar los resultados de las evaluaciones que completan los jugadores, incluyendo puntaje y estado de completitud.

| Id   | Requerimiento/Historia                 | Caso de Prueba                                   | Descripción                                             | Precondiciones                     | Área Funcional | Pasos a Seguir                                 | Datos/Acciones de Entrada                                            | Resultado Esperado                                                                 | Dependencias    | Responsable |
| ---- | -------------------------------------- | ------------------------------------------------ | ------------------------------------------------------- | ---------------------------------- | -------------- | ---------------------------------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | --------------- | ----------- |
| RE01 | Listar resultados                      | test_RE01_listar_resultados                      | Obtiene todos los resultados de evaluaciones            | \* API disponible                  | CRUD Resultado | 1. GET a `/api/resultado/`                     | Ninguno                                                              | Status 200, array de resultados                                                    | Ninguna         | QA Team     |
| RE02 | Crear resultado completo               | test_RE02_crear_resultado_completo               | Crea resultado con todos los campos                     | \* Evaluación y usuario existentes | CRUD Resultado | 1. POST a `/api/resultado/`                    | `{"evaluacion": 1, "usuario": 1, "puntaje": 85, "completado": true}` | Status 201, mensaje="Resultado creado correctamente.", puntaje=85, completado=true | Evaluación, J02 | QA Team     |
| RE03 | Crear resultado valores por defecto    | test_RE03_crear_resultado_valores_por_defecto    | Crea resultado sin puntaje ni completado (usa defaults) | \* Evaluación y usuario existentes | CRUD Resultado | 1. POST sin puntaje ni completado              | `{"evaluacion": 1, "usuario": 1}`                                    | Status 201, puntaje=20 (default), completado=false (default)                       | Evaluación, J02 | QA Team     |
| RE04 | Crear resultado evaluación inexistente | test_RE04_crear_resultado_evaluacion_inexistente | Rechaza cuando la evaluación no existe                  | \* Usuario existente               | CRUD Resultado | 1. POST con evaluacion=99999                   | `{"evaluacion": 99999, "usuario": 1, "puntaje": 50}`                 | Status 400 Bad Request                                                             | J02             | QA Team     |
