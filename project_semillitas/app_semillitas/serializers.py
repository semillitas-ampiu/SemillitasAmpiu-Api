    ...
    def validate(self, data):
        ...
        if not username:
            raise serializers.ValidationError(
                {"email": "El correo es obligatorio para el Administrador."}
            )
...
class JugadorSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not jugador_nombre:
            raise serializers.ValidationError(
                {"username": "El nombre de usuario es obligatorio para el Jugador."}
            )
...
class UsuarioPalabrasSerializer(serializers.ModelSerializer):
    palabra = serializers.PrimaryKeyRelatedField(
        queryset=Palabra.objects.all(), required=True
    )
...
class ResultadoEvaluacionesSerializer(serializers.ModelSerializer):
    usuario = serializers.PrimaryKeyRelatedField(
        queryset=Usuario.objects.all(), required=True
    )
